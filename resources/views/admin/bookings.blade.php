...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
    <div class="container mx-auto py-8 px-4">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600">View and manage all room bookings across the system</p>
        </div>
        
        @if(session('success'))
            <div class="mb-6 bg-gradient-to-r from-green-50 to-green-100 border-l-4 border-green-500 text-green-800 px-6 py-4 rounded-lg shadow-sm flex items-center">
        ...
        <div class="bg-white rounded-xl shadow-md p-6 mb-6 border border-gray-200">
            <div class="flex items-center mb-4">
                ...
                <h2 class="text-lg font-semibold text-gray-900">Filter Bookings</h2>
            </div>
            <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="date" name="date" value="{{ $date }}" 
                           class="w-full border-2 border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        <svg class="w-4 h-4 inline-block mr-1 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <select name="room_id" class="w-full border-2 border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all">
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
            <div class="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                    ...
                    <span class="ml-auto text-sm font-normal text-gray-600">Total: {{ $bookings->count() }}</span>
                </h3>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Booking Details</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Room & Schedule</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">User Information</th>
                        ...
                        @forelse($bookings as $booking)
                            <tr class="hover:bg-blue-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-xs text-gray-600 flex items-center ml-10">
                                    ...
                                    <div class="text-xs text-gray-600 flex items-center ml-10 mt-1">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">{{ optional($booking->user)->name ?? '—' }}</div>
                                            <div class="text-xs text-gray-500">{{ optional($booking->user)->email ?? '—' }}</div>
                                        </div>
                                    </div>
                                </td>
                                    ...
                                    <form method="POST" action="{{ route('admin.bookings.status', $booking) }}" class="inline">
                                        ...
                                        <select name="status" 
                                                ...
                                                class="text-sm font-semibold rounded-lg px-4 py-2 border-2 focus:outline-none focus:ring-2 transition-all
                                                       {{ $booking->status === 'pending' ? 'bg-yellow-50 text-yellow-800 border-yellow-300 hover:bg-yellow-100 focus:ring-yellow-200' : '' }}
                                                       {{ $booking->status === 'confirmed' ? 'bg-green-50 text-green-800 border-green-300 hover:bg-green-100 focus:ring-green-200' : '' }}
                                                       {{ $booking->status === 'rejected' ? 'bg-red-50 text-red-800 border-red-300 hover:bg-red-100 focus:ring-red-200' : '' }}">
                                ...
                                <td class="px-6 py-4">
                                    <form method="POST" action="{{ route('admin.bookings.delete', $booking) }}" class="inline" 
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-red-50 text-red-700 text-sm font-medium rounded-lg border-2 border-red-200 hover:bg-red-100 hover:border-red-300 transition-all duration-200 shadow-sm hover:shadow">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-gray-500 text-sm">No bookings match the selected filters.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
